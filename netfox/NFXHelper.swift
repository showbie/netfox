...
extension URLResponse
{
    ...
    func getNFXHeaders() -> Dictionary<AnyHashable, AnyObject>
    {
        return (self as? HTTPURLResponse)?.allHeaderFields as [AnyHashable: AnyObject]? ?? [AnyHashable: AnyObject]()
    }
}
...
class NFXDebugInfo {
    ...
    {
        let req = NSMutableURLRequest(url: URL(string: "https://api.ipify.org/?format=json")!)
        ...
        session.dataTask(with: req as URLRequest, completionHandler: { (data, response, error) in
            do {
                let rawJsonData = try JSONSerialization.jsonObject(with: data!, options: [.allowFragments]) as AnyObject
